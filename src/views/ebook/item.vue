...
<template>
  <div class="product_item" @mousemove="mousemove">
    <q-card class="q-pa-md" flat bordered>
      <div class="column flex justify-center items-center">
        <q-img  class="product_item_img" :src="bookItem.imageUrl" />
        <div class="q-pa-xs product_item_min_title">{{bookItem.productName}}</div>
        <div class="q-pa-xs product_item_title">{{bookItem.productName}}</div>
        <div class=" row q-pa-xs ">
          <q-btn class="q-ma-xs" outline rounded dense label="14" icon-right="message" />
          <q-btn class="q-ma-xs" outline rounded dense label="14" icon-right="message" >
            (166)
          </q-btn>
        </div>
        <div class="row q-pa-xs row ">
          <q-btn class="col-6"  >
            <q-badge color="gary" class="product_item_text_del_line" style="background-color: #6C7486">
              $199
            </q-badge>
            <div class="q-ma-xs product_item_text text-red">${{bookItem.price}}</div>
          </q-btn>
          <button @click="$router.push({path:'/details',query:{productId:bookItem.productId}})" >
